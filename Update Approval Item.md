...
Use this API to update an existing approval item in the approval center server.

Note:

* This API requires **Update** **Approval Item** permission and the relevant **Data Scope**
...
**End Point**: https://openapi.seatalk.io/approval\_center/v2/update
...
|created\_at|int|No|int64|The timestamp when the approval item was created|||
...
|applicant\_name|string|No|50 char per language|The name of the applicant for this approval item|||
|has\_attachments|bool|No||An icon to show if the approval item has any attachments. For display only|||
|item\_name|TextStructure|No|50 char per language|- The name of the item type for display - The app name will be displayed if this field is not passed in|The current App's name|"Sick Leave"|
...
|title|TextStructure|No|100 char per language|The title of the approval item|""|"Take 3 Day Sick Leave"|
...
|action\_button|int|No||The actions the user can perform on this approval item directly in the list on the Pending/Snooze tab. - 1: Approve + Reject + Snooze - 2: View Details + Snooze|||
|app\_path|string|No|500 char|- The app path to the detailed page of the approval item - This field must have a non-empty value if the action\_button field is set to 2 - Follow this link format: seatalk://application/sop/<app\_id>/<app\_path>?itemId=xxxx|||
|approve\_url|string|No|500 char|- A callback URL on your app's server. When the user approves this item in Approval Center, we will send the following info to this callback URL in the request Body (JSON): { "item\_id": "item's-id", " reason ": "", "employee": { "employee\_code": "e\_12345", " name" : "Jason", " email" : "jasonsemail@domain.com" } } - A callback request can be verified by comparing a local signature and remote one. See Approval Center Callback Signature Verification for detail|||
|reject\_url|string|No|500 char|- A callback URL on your SOP App's server. When the user rejects this item in Approval Center, we will send the following info to this callback URL in the request Body (JSON): { "item\_id": "item's-id", "reason": "Reason for rejecting", "employee": { "employee\_code": "e\_12345", "name": "Jason", "email": "jasonemail@domain.com" } } - A callback request can be verified by comparing a local signature and remote one. See Approval Center Callback Signature Verification for detail|||
