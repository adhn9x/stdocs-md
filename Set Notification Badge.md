...
|∟employee\_code|string|Yes||The employee\_code of the user||
...
|failed\_offsets|\[\]int|The failed recipients offsets (starting from 0)|
|failed\_reasons|\[\]string|- EMPLOYEE\_ID\_NOT\_FOUND: employee\_code is not valid - VERSION\_OUTDATED: the request is superseded by another request with a higher version - APP\_REGISTRATION\_REQUIRED: the employee has not registered on the SeaTalk App yet|
